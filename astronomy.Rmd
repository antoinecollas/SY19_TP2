...
if (RECHERCHE_HYPERPARAMETRES){
  set.seed(123)
  C_liste <- (1:5)^5
  erreurs <- rep(0, length(C_liste))
  for (i in 1:length(C_liste)){
    hyperparametres <- list(kernel="vanilla", C=C_liste[i], kpar=list())
    erreurs[i] <- CV_eval('svc', astronomy_train, hyperparametres)[1]
  }
  plot(C_liste, erreurs, type="l",xlab="C", ylab="Erreur")
  C <- C_liste[which.min(erreurs)]
  print(C)
}else{
  C <- 3125
}
...
if (RECHERCHE_HYPERPARAMETRES){
  set.seed(123)
  degres <- 2:5 
  erreurs <- rep(0, length(degres)) 
  for (i in 1:length(degres)){
    hyperparametres <- list(kernel="polydot", kpar=list(degree=degres[i]), C=C)
    erreurs[i] <- CV_eval('svc', astronomy_train, hyperparametres)[1]
  }
  plot(degres, erreurs, type="l",xlab="Degre", ylab="Erreur")
  degre <- degres[which.min(erreurs)]
  print(degre)
}else{
  degre <- 2
}
...
if (RECHERCHE_HYPERPARAMETRES){
  set.seed(123)
  sigma <- seq(0.0001, 0.02, by=0.005)
  erreurs <- rep(0, length(sigma)) 
  for (i in 1:length(sigma)){
    hyperparametres <- list(kernel="rbfdot", kpar=list(sigma=sigma[i]), C=C)
    erreurs[i] <- CV_eval('svc', astronomy_train, hyperparametres)[1]
  }
  plot(sigma, erreurs, type="l",xlab="Sigma", ylab="Erreur")
  sigma <- sigma[which.min(erreurs)]
  print(sigma)
}else{
  sigma <- 0.005
}
...
Nous observons que le noyau gaussien n'améliore pas non plus les perfomances par rapport à un noyau linéaire.
...
Nous devons tout d'abord nous assurer que notre fôret aléatoire contient suffisamment d'arbres. En effet, si le nombre d'arbres est trop faible certains individus ne seront pas utilisés (à cause du bootstrap). Si le nombre d'arbres est élévé la recherche de l'hyperparamètre 'mtry' (que faisons ensuite) sera très longue. Pour cela nous calculons l'erreur de classification par validation croisée pour des fôrets ayant entre 1 et 200 (par pas de 20).

```{r, fig.height = 3, fig.width = 7, fig.align = "center", echo=FALSE}
if (RECHERCHE_HYPERPARAMETRES){
  set.seed(123)
  ntree <- c(1, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200)
  erreurs <- rep(0, length(ntree)) 
  for (i in 1:length(ntree)){
    hyperparametres <- list(ntree=ntree[i])
    erreurs[i] <- CV_eval('rf', astronomy_train, hyperparametres)[1]
  }
  plot(ntree, erreurs, type="l",xlab="Nombre d'arbres", ylab="Erreur")
  ntree <- ntree[which.min(erreurs)]
  print(ntree)
}else{
  ntree <- 160
}
...
if (RECHERCHE_HYPERPARAMETRES){
  ...
  mtry <- 1:nb_vars
  erreurs <- rep(0, nb_vars) 
  for (i in 1:nb_vars){
    hyperparametres <- list(mtry=mtry[i], ntree=ntree)
    erreurs[i] <- CV_eval('rf', astronomy_train, hyperparametres)[1]
  }
  plot(mtry, erreurs, type="l",xlab="Nombre de variables", ylab="Erreur")
